...
def run_builder(output_file='UKCH20201018.csv'):
    ...
    with open(output_file, 'w', newline='', encoding="utf-8") as csvfile:
        ...
        for cat_art_num in catalysis_articles.keys():
            print(catalysis_articles[cat_art_num])
            writer.writerow(catalysis_articles[cat_art_num])


def build_pub_list(argv):
    print('Argument List:', argv)
    csv_out = ""
    try:
        csv_out = argv[0]
    except:
        print("provide name of output file")
        return
    run_builder(csv_out)

if __name__ == "__main__":
   build_pub_list(sys.argv[1:])
