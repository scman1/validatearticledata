    ...
    for affi_text in new_affis:
        if affi_text  != "":
            ...
            for afi_key in affi_records:
                ...
                matches = 0
                for field in affi:        
                    if affi_text.find(affi[field]) != -1 and field != 'ID' \
                       and affi[field]!='':
                        matches +=1
                if matches > 2:
                    likely_affis[afi_key]=matches
            print('***************************************************************')
            print(affi_text)
            if len(likely_affis) == 0:
                ...
                for affi_id in likely_affis:
                    v_aff = str(affi_id)+"| "
                    for key in affi_records[affi_id]:
                        if affi_records[affi_id][key] != "":
                            v_aff += ", "+ affi_records[affi_id][key]
                    options[i_aff] = v_aff
                    i_aff += 1
